...
// ── COURSE DATA ────────────────────────────────────────────────────────────
// NOTE: No hardcoded courses. Courses should be fetched from `courses` Supabase table.
// For now, empty arrays to show proper empty state.
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      <div className="bg-lumina-card border border-lumina-border rounded-lg p-4">
        ...
        <p className="text-lumina-dim text-xs mt-1">{COURSES.length === 0 ? 'Pending...' : 'All Free'}</p>
      </div>

      <div className="bg-lumina-card border border-lumina-border rounded-lg p-4">
        <p className="text-lumina-muted text-xs font-semibold uppercase mb-1">Estimated Value</p>
        <p className="text-2xl font-bold text-lumina-gold">{COURSES.length === 0 ? '--' : '$50k+'}</p>
        <p className="text-lumina-dim text-xs mt-1">{COURSES.length === 0 ? 'No data' : 'In content'}</p>
      </div>
      ...
      <div className="bg-lumina-card border border-lumina-border rounded-lg p-4">
        ...
        <p className="text-2xl font-bold text-lumina-text">{inProgressCourses}/{COURSES.length || '0'}</p>
        ...
        <section data-courses-section>
          ...
          {filteredCourses.length === 0 ? (
            <div className="bg-lumina-card border border-lumina-border rounded-lg p-12 text-center">
              <BookOpen size={48} className="mx-auto mb-4 text-lumina-muted" />
              <h3 className="text-xl font-bold text-lumina-text mb-2">AI Education Hub is Being Configured</h3>
              <p className="text-lumina-muted mb-4">
                Courses will appear here when available. Data will be populated from the Supabase courses table.
              </p>
              <p className="text-sm text-lumina-dim">
                Set up a courses table with structured learning resources to get started.
              </p>
            </div>
          ) : (

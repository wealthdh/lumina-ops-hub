 ...
 * Auto-synthetic trades Â· real-time signal detection

 *

 * ALL DATA LIVE â no mock/hardcoded data.
 ...
 * When no MT5 positions exist, returns empty â detectArbitrage finds no cross-market edges.
...
export default function EdgeHarmonizer() {
  ...
  // ââ LIVE data from Supabase âââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  const polyOptions = markets.slice(0, 6).map((m) => {
    ...
    return { label: `${m.question.slice(0, 40)}â¦ YES @ ${Math.round(yesPrice * 100)}Â¢`, id: m.id }
  ...
  function scan() {
    ...
    console.log('[EdgeHarmonizer] Scanning marketsâ¦', { marketsCount: markets.length, mt5TradesCount: mt5Trades.length })
    ...
    setTimeout(() => {
      ...
      if (freshSignals.length > 0) {
        ...
        setScanResult('Scan complete â no open MT5 positions detected. Open trades in LuminaPulse MT5 to enable cross-market edge detection.')

      } else {

        setScanResult(`Scan complete â ${markets.length} markets checked, ${mt5Trades.length} MT5 positions analyzed. No edges above 3% threshold right now.`)
  ...
  function buildSynthetic() {
    ...
    console.log('[EdgeHarmonizer] Building syntheticâ¦', { selectedPoly, selectedInstrument, capital })
    ...
    if (!market) {
      ...
      setBuildResult('â  No Polymarket data available â cannot compute synthetic')
    ...
    if (edge < 3) {
      ...
      setBuildResult(`Edge ${edge.toFixed(1)}% is below 3% threshold â trade not recommended. Adjust parameters or wait for better conditions.`)

    } else {

      setBuildResult(`â Synthetic built: ${market.question.slice(0, 50)}â¦ Ã ${selectedInstrument} | Edge: +${edge.toFixed(1)}% | Kelly: ${kelly.toFixed(2)} | Max Loss: -$${maxLoss} | Capital: $${capital.toLocaleString()}. Connect MT5 to execute.`)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div>
          ...
          <p className="text-lumina-dim text-sm">Cross-Market Arbitrage Bridge Â· auto-synthetic trade execution</p>
      ...
      {autoMode && (
        ...
        <div className="p-3 bg-lumina-pulse/10 border border-lumina-pulse/30 rounded-lg flex items-center gap-2 text-sm">
          ...
          <span className="text-lumina-dim">â signals â¥ $500 capital & â¥3% edge execute automatically</span>
        ...
        {signals.length === 0 ? (
          ...
          <div className="text-center text-lumina-dim py-8 text-sm">
            ...
            {mt5Trades.length === 0
              ...
              ? 'No open MT5 positions â start trading to detect cross-market edges'
              ...
              <div key={s.marketId} className="p-4 bg-lumina-bg/60 rounded-xl border border-lumina-pulse/20 hover:border-lumina-pulse/40 transition-colors">
                ...
                <div className="grid grid-cols-4 gap-2 mb-3 text-xs">
                  ...
                  <div>
                    ...
                    <div className="text-lumina-text font-mono">{(s.polyPrice * 100).toFixed(0)}Â¢</div>
      ...
      {/* Synthetic trade builder â uses live Polymarket data */}
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              >
                ...
                {polyOptions.length > 0 ? (
                  ...
                  <option disabled>Loading marketsâ¦</option>
            ...
            <div>
              ...
              >
                ...
                <option value="EURUSD">EURUSD (BUY â rate cut bullish)</option>

                <option value="XAUUSD">XAUUSD (BUY â risk-off hedge)</option>

                <option value="USDJPY">USDJPY (SELL â rate diff)</option>
          ...
          <div className="space-y-3">
            ...
            <div className="p-3 bg-lumina-pulse/10 border border-lumina-pulse/20 rounded-lg">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs font-mono">
                ...
                <div><div className="text-lumina-dim">Edge</div><div className="text-lumina-pulse">{customMetrics ? customMetrics.edge : signals[0] ? `+${signals[0].edgePct.toFixed(1)}%` : 'â'}</div></div>

                <div><div className="text-lumina-dim">Kelly</div><div className="text-lumina-gold">{customMetrics ? customMetrics.kelly : signals[0] ? (signals[0].confidence / 800).toFixed(2) : 'â'}</div></div>

                <div><div className="text-lumina-dim">Max Loss</div><div className="text-lumina-danger">{customMetrics ? customMetrics.maxLoss : signals[0] ? `-$${Math.round(signals[0].suggestedCapital * 0.048)}` : 'â'}</div></div>
      ...
      {/* Signal history â live from Supabase arbitrage_signals table */}
        ...
        <div className="space-y-2">
          ...
          {signalHistory.length === 0 ? (
            ...
            <div className="flex flex-col items-center py-6 text-lumina-dim text-sm gap-2">
              ...
              No executed signals yet â signals appear here once trades fire
            ...
            signalHistory.map((s: Record<string, unknown>) => (
              ...
              <div key={String(s.id)} className="flex items-center gap-3 p-3 bg-lumina-bg/60 rounded-lg">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-lumina-dim capitalize">{String(s.type ?? '')} Â· {String(s.status ?? '')}</div>

/**
 * Twin-Engine Dashboard â Polymarket + MT5 split screen with one-tap Mirror Edge
 */
import { useState } from 'react'
import { useQuery } from '@tanstack/react-query'
import { Zap, RefreshCw, TrendingUp, TrendingDown, ExternalLink, ChevronDown, ChevronUp, AlertTriangle, Link2, Radio } from 'lucide-react'
...
// âââ Real equity curve from Supabase mt5_snapshots âââââââââââââââââââââââââââ
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="card-glow w-full max-w-md border-lumina-pulse/40">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lumina-text font-semibold">Mirror Edge â One-Tap Sync</h2>
        </div>
          ...
          <div className="space-y-4">
            <div className="p-3 bg-lumina-bg/60 rounded-lg">
              ...
              <div className="flex items-center gap-4 mt-2">
                <span className="badge-pulse badge">YES @ 62Â¢</span>
            ...
            <div className="p-3 bg-lumina-bg/60 rounded-lg">
              ...
              <div className="text-sm text-lumina-text">EURUSD BUY â 0.3 lot</div>
            ...
            <div className="p-3 bg-lumina-pulse/10 border border-lumina-pulse/30 rounded-lg text-xs text-lumina-text">
              Edge and risk will be calculated from live market data once positions are entered.
            </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2 text-sm">
              {[
                ...
                ['MT5 Hedge:',  'Pending â will calculate from live feeds'],
        ...
        {step === 'done' && (
          <div className="text-center space-y-4 py-4">
            ...
            <div className="text-xs text-lumina-dim">
              Order submitted. Check Polymarket positions and MT5 terminal for confirmation.
            </div>
        ...
        {step !== 'done' && (
          <button onClick={onClose} className="absolute top-3 right-3 text-lumina-dim hover:text-lumina-text text-lg">â</button>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2">
      {trades.map((t) => {
        ...
        // swap / openTime may not exist on all mock types â safe fallback
...
export default function TwinEngine() {
  ...
  // ââ LIVE data from Supabase / MT5 bridge ââââââââââââââââââââââââââââââââââ
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="card-glow border-lumina-violet/30">
          ...
          <div className="space-y-3">
            {marketsLoading && markets.length === 0 ? (
              <div className="text-center py-8 text-lumina-dim text-xs animate-pulse">Loading live marketsâ¦</div>
                ...
                <a key={m.id}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-3 text-xs font-mono">
                      <span className="text-lumina-success">YES {((yes?.price ?? 0) * 100).toFixed(0)}Â¢</span>
                      <span className="text-lumina-danger">NO {((no?.price ?? 0) * 100).toFixed(0)}Â¢</span>
                    </div>
                  </div>
        ...
        <div className="card-glow border-lumina-gold/30">
          ...
          {/* Equity curve â real from mt5_snapshots table */}
          ...
          {/* Open trades â expandable */}
          <MT5TradeList trades={account.openTrades} />
        </div>
      </div>

      {/* Live Arbitrage Signals from Edge Harmonizer */}
      <EdgeHarmonizerSignalFeed onExecute={() => setShowMirror(true)} />
    </div>
  )
}

// âââ Signal Feed from Edge Harmonizer engine âââââââââââââââââââââââââââââââââ

function EdgeHarmonizerSignalFeed({ onExecute }: { onExecute: () => void }) {
  const signals = useSignalStore((s) => s.activeSignals)
  const lastPushAt = useSignalStore((s) => s.lastPushAt)
  const markSentToTwin = useSignalStore((s) => s.markSentToTwin)

  if (signals.length === 0) {
    return (
      <div className="card-glow border-lumina-border p-4 flex items-center gap-3">
        <Radio size={14} className="text-lumina-dim" />
        <div>
          <div className="text-sm text-lumina-dim font-medium">Waiting for Edge Harmonizer signals...</div>
          <div className="text-xs text-lumina-muted">
            {lastPushAt
              ? `Last signal: ${new Date(lastPushAt).toLocaleTimeString()}`
              : 'Autonomous scan runs on Edge Harmonizer tab load + every 60 min'}
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="space-y-2">
      {signals.slice(0, 3).map((sig) => (
        <div key={sig.id} className="card-glow border-lumina-pulse/30 p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="pulse-dot" />
            <div>
              <div className="text-sm text-lumina-text font-semibold">
                {sig.question.length > 50 ? sig.question.slice(0, 50) + '...' : sig.question}
              </div>
              <div className="text-xs text-lumina-dim">
                Poly {(sig.polyPrice * 100).toFixed(0)}c vs MT5 implied {(sig.mt5ImpliedProb * 100).toFixed(0)}% -
                Edge: <span className="text-lumina-pulse">+{sig.edgePct.toFixed(1)}%</span> -
                Capital: <span className="text-lumina-gold">${sig.suggestedCapital.toLocaleString()}</span>
              </div>
            </div>
          </div>
          <button
            className="btn-pulse text-sm flex items-center gap-2"
            onClick={() => {
              markSentToTwin(sig.marketId)
              onExecute()
            }}
          >
            <Zap size={14} />

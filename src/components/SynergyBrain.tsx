...
import { GitBranch, TrendingUp, CheckCircle, Loader } from 'lucide-react'
...
function useCreateSynergy() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async (opts: { jobA: string; jobB: string; value: number; description: string }) => {
      const { error } = await supabase
        .from('synergy_links')
        .insert({
          job_a: opts.jobA,
          job_b: opts.jobB,
          synergy_type: 'ai-discovered',
          value: opts.value,
...
export default function SynergyBrain() {
  ...
  const [activatedPairs, setActivatedPairs] = useState<Set<string>>(new Set())
            ...
            return [(
              <div key={`${jobA.id}-${jobB.id}`} className="p-3 bg-lumina-bg/60 rounded-lg hover:bg-lumina-bg transition-colors">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {activatedPairs.has(`${jobA.id}-${jobB.id}`) ? (
                    <span className="text-xs text-lumina-success flex items-center gap-1"><CheckCircle size={10} /> Active</span>
                  ) : (
                    <button
                      ...
                      disabled={createSynergy.isPending}
                      onClick={() => {
                        createSynergy.mutate({
                          jobA: jobA.id,
                          jobB: jobB.id,
                          value,
                          description: `AI-discovered synergy: ${jobA.name} Ã ${jobB.name}`,
                        }, {
                          onSuccess: () => setActivatedPairs(prev => new Set(prev).add(`${jobA.id}-${jobB.id}`)),
                        })
                      }}
                    >
                      {createSynergy.isPending ? 'Saving...' : 'Activate'}
                    </button>
                  )}
                </div>
...
// Kill recommendations — populated by real Synergy Brain analysis only

...
const log = (level, msg, data) => {
  ...
  console[level](`[create-checkout-session][${ts}] ${msg}`, data ? JSON.stringify(data) : '');
};
...
export default async function handler(req, res) {
  ...
  if (!stripeKey || stripeKey === 'sk_live_...' || stripeKey === 'sk_test_...') {
    ...
    return res.status(500).json({
      ...
      details: 'STRIPE_SECRET_KEY is missing or placeholder.',
  ...
  try {
    ...
    if (error.type === 'StripeAuthenticationError') {
      return res.status(401).json({
        ...
        details: 'Your STRIPE_SECRET_KEY is invalid.',
      });
    }

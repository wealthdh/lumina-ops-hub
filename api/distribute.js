/**
 * Vercel Serverless Function â Auto-Distribution Engine
 *
 ...
 *   POST ?action=distribute   â Distribute a creative to all connected platforms
 *   POST ?action=single       â Distribute to a single platform
 *   GET  ?action=status       â Get distribution status for a creative
 *   GET  ?action=connections   â List platform connection status
 */
import { createClient } from '@supabase/supabase-js'
import crypto from 'crypto'

// âââ Supabase (server-side) âââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Platform Distributors ââââââââââââââââââââââââââââââââââââââââââââââââââ
...
const DISTRIBUTORS = {
  ...
  async Instagram(creative, connection) {
    // Instagram Graph API â Container + Publish flow
    if (!connection?.access_token) {
  ...
  async YouTube(creative, connection) {
    ...
    // For now, queue as pending â real upload needs multipart
    return {
      success: false,
      error: 'YouTube Shorts upload requires OAuth flow â queued for manual review',
    }
  },
  ...
  async 'Twitter/X'(creative, connection) {
    // Twitter/X v2 API â real tweet posting
    // Supports two auth modes:
    //   A) OAuth 2.0 User Context (from platform_connections table)
    //   B) OAuth 1.0a App+User tokens (from env vars â fallback)
    ...
    try {
      ...
      if (useOAuth2) {
        // ââ OAuth 2.0 User Context (Bearer token from platform_connections) ââ
      ...
      // ââ OAuth 1.0a (env var credentials) âââââââââââââââââââââââââââââââââââ
  ...
  async Pinterest(creative, connection) {
    ...
    return {
      success: false,
      error: 'Pinterest video pin API integration pending â queued',
    }
  },
...
// âââ Fetch timeout wrapper âââââââââââââââââââââââââââââââââââââââââââââââââââ
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ââ Distribute to ALL connected platforms ââââââââââââââââââââââ
    if (action === 'distribute' && req.method === 'POST') {
      ...
      if (!creative.video_url) {
        return res.status(400).json({ error: 'Creative has no video_url â generate first' })
      }
      ...
      // Update creative status â only 'distributed' if at least one succeeded
    ...
    // ââ Distribute to a SINGLE platform ââââââââââââââââââââââââââââ
    ...
    // ââ Get distribution status ââââââââââââââââââââââââââââââââââââ
    ...
    // ââ List platform connections ââââââââââââââââââââââââââââââââââ

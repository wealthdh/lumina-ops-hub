/**
 * Stripe Webhook Handler — REAL payment processing
 *
 ...
 *
 * NEVER fakes success — only confirms after Stripe signature is verified.
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // ── 1. Match to a product in stripe_products ─────────────────
      let productId = null;
      let productName = null;

      // Method A: Match via session metadata (set by create-checkout-session)
      if (session.metadata?.supabase_product_id) {
        productId = session.metadata.supabase_product_id;
        productName = session.metadata.product_name || null;
        log('info', 'Matched via session metadata', { productId, productName });
      }

      // Method B: Match via line items → stripe_products table
      if (!productId) {
        ...
        if (lineItems?.data?.length > 0) {
          ...
          if (stripePriceId || stripeProductId) {
            ...
            if (matchedProduct) {
              ...
              productName = matchedProduct.product_name || productName;
              log('info', 'Matched via line items', { productId, stripePriceId });
            } else {
              log('warn', 'No matching product in stripe_products', { stripePriceId, stripeProductId });
            }
          }
        }
      }
      ...
      if (orderError) {
        // Duplicate session_id means we already processed this — that's OK
        if (orderError.code === '23505') {
          log('warn', 'Duplicate order — already processed', { session_id: session.id });
        } else {
      ...
      // ── 3. Insert into `income_entries` for revenue tracking ─────
      ...
      const { error: incomeError } = await supabase.from('income_entries').insert({
        ...
        amount: amountDollars,
        currency,
        description: `Stripe payment: ${buyerEmail} — ${lineItems?.data?.[0]?.description || session.id}`,
        metadata: {
          stripe_session_id: session.id,
